...
class DiffGtMel:
    ...
    def check_args(self, args1, args2):
        if args1.data.block_size != args2.data.block_size:
            raise ValueError("DDSP and DIFF models have inconsistent block sizes.")
        if args1.data.sampling_rate != args2.data.sampling_rate:
            raise ValueError("The sampling rates of the DDSP and DIFF models are inconsistent.")
        if args1.data.encoder != args2.data.encoder:
            raise ValueError("The encoders of the DDSP and DIFF models are inconsistent.")
        return True
